...
// Hand-edited for P81-124405 (BUG-26 follow-up, BUG-25 family). The swagger
// declares passphrase + remoteID as required on IPSecRedundantTunnel, but
// the public-api GET redundant-tunnel response omits passphrase (it's a
// secret — not returned on Read) and may also omit remoteID when the user
// didn't set one. The codegen-generated UnmarshalJSON strict-checked the
// full required list and rejected every Read with
// `no value given for required property passphrase`, leaving terraform
// with `Root resource was present, but now absent`.
// We trim requiredProperties to fields the upstream wire always emits:
